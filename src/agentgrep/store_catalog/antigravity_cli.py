...
_ANTIGRAVITY_CLI_STORES: tuple[StoreDescriptor, ...] = (
    ...
    StoreDescriptor(
        ...
        schema_notes=(
            ...
            "`created_at`. A string `content` carries user/assistant turns; some "
            "lines add readable payload in `thinking`/`tool_calls` (which may "
            "co-occur with `content`, not only replace it); and some lines "
            "(e.g. `CONVERSATION_HISTORY`) carry none of `content`/`thinking`/"
            "`tool_calls` (an occasional `error` key also appears). This is the "
