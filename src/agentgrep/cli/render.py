...
def _format_insight_family_lines(
    ...
    *,
    colors: agentgrep.AnsiColors,
    item_formatter: cabc.Callable[[object, agentgrep.AnsiColors], str],
) -> list[str]:
    ...
    if not isinstance(items, cabc.Sequence) or isinstance(items, (str, bytes, bytearray)):
        return lines
    shown = 0
    for item in items[:_HUMAN_SAMPLE_LIMIT]:
...
def _format_variant_edge_line(item: object, colors: agentgrep.AnsiColors) -> str:
...
def _format_omission_finding_line(item: object, colors: agentgrep.AnsiColors) -> str:
...
def _format_suggestion_text(payload: object, *, colors: agentgrep.AnsiColors) -> str:
...
class ConsoleInsightsAnalyzeProgress:
    ...
    def _render_tty(self, frame: str) -> None:
        ...
        if summary_lines:
            first_width = max(1, terminal_width - agentgrep._visible_width(frame_text) - 1)
            summary_lines[0] = agentgrep._hard_truncate_ansi(summary_lines[0], first_width)
...
def format_insights_analyze_progress_lines(
    ...
    *,
    ...
    if max_width is None:
        return lines
    return tuple(agentgrep._hard_truncate_ansi(line, max_width) for line in lines)
