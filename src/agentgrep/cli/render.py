...
def run_search_command(args: SearchArgs) -> int:
    ...
    if listener is not None:
        listener.start()
    try:
        run = _run_search_result_for_cli(
            pathlib.Path.home(),
            query,
            progress=progress,
            control=control,
...
def _run_search_result_for_cli(
    home: pathlib.Path,
    query: SearchQuery,
    *,
    ...
    """Run a result-producing CLI search with scoped DB ownership."""
    from agentgrep.db import DbQueryUnsupportedError

    runtime = _db_runtime_for_cli(cache_mode)
    runner = _facade().run_search_result
    try:
        if runtime is None or not _accepts_runtime_parameter(runner):
            return t.cast(
                "SearchResult",
                runner(home, query, progress=progress, control=control),
            )
        return t.cast(
            "SearchResult",
            runner(
