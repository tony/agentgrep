...
def _effective_export_scope(
    namespace: argparse.Namespace,
    *,
    user_ast: QueryNode | None,
) -> SearchScope:
    """Return export's coarse scope after query-language reconciliation."""
    explicit = t.cast("SearchScope | None", namespace.scope)
    base_scope: SearchScope = "prompts" if explicit is None else explicit
    if user_ast is None:
        return base_scope
    from agentgrep.query import scope_widened_for_ast

    return scope_widened_for_ast(user_ast, base_scope)
...
def _build_export_args(
    ...
    *,
    ...
    compiled, residual_terms, user_ast, _diagnostics = _maybe_compile_query(
        terms_list,
        bundle=bundle,
        color_mode=color_mode,
    ...
    return ExportArgs(
        ...
        agents=agents,
        scope=_effective_export_scope(namespace, user_ast=user_ast),
        case_sensitive=t.cast("bool", namespace.case_sensitive),
        limit=limit,
