...
class CompiledQuery:
    ...
    is_pure_text : bool
        ...
        are ``None``.
    field_predicate : t.Callable[[SearchRecord], bool] | None
        Field-only predicate when text matching can be separated without changing
        boolean semantics. ``None`` when there is no separable field-only AST.
    field_filter_safe : bool
        Whether applying ``field_predicate`` without the text terms preserves boolean
        semantics.
    """
