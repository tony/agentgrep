...
|-------|----------------|----------------|----------------------|--------|-----------------------|----------------------------------|---------------------|--------------------|---------------------------|
| Codex | {doc}`codex` history and modern/legacy sessions | {doc}`codex` state DB previews, instructions, memory workspace, memory DB, goals DB, session index, skills, rules, plugins | {doc}`codex` logs DB feedback bodies, external import ledger, config/app-state key summaries | {doc}`codex` memory workspace and DB | {doc}`codex` goals DB | {doc}`codex` instructions, plugins, skills, rules | {doc}`codex` `session_index.jsonl` | {doc}`codex` config, config backups, model/internal/update/version/process state | {doc}`codex` logs, cache, SQLite sidecars, shell snapshots, auth/policy/installation id |
| Claude | {doc}`claude` history, project sessions, subagents | {doc}`claude` `__store.db`, project/session memory, tasks, todos, plans, skills, commands, teams | {doc}`claude` settings/keybindings and app-state key summaries | {doc}`claude` project/session memory | {doc}`claude` tasks, todos, and plans | {doc}`claude` skills, commands, teams, plugin cache | | {doc}`claude` settings, sessions, update state, context/security, IDE state | {doc}`claude` paste/image cache, file history, shell snapshots, debug, backups, credentials/session env |
