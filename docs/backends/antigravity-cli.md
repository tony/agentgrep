...
`created_at`; string `content` holds user/assistant turns but is absent
on tool/thinking-only lines, where the payload lives in `tool_calls` or
`thinking` instead. This is the readable counterpart to
