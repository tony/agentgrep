/* CLI install widget: persist method selection across the docs site.
 *
 * Storage key: agentgrep.cli-install.method. Mirrors onto
 * <html data-cli-install-method=...> so the CSS in widget.css drives
 * panel visibility. The accompanying prehydrate snippet
...
(function () {
  ...
  var STORAGE_KEY = "agentgrep.cli-install.method";
  var ATTR = "data-cli-install-method";
  var SYNC_EVENT = "agentgrep:cli-install:sync";

  function widgets() {
    return document.querySelectorAll(".ag-cli-install");
  }
  ...
  function syncHtmlAttr() {
    var first = document.querySelector(".ag-cli-install");
    if (!first) return;
  ...
  function select(widget, value, opts) {
    ...
    var tab = widget.querySelector(
      '.ag-cli-install__tab[data-tab-value="' + value + '"]'
    );
    if (!tab) return;
    var tabs = widget.querySelectorAll(".ag-cli-install__tab");
    tabs.forEach(function (t) {
      var on = t === tab;
      t.setAttribute("aria-selected", on ? "true" : "false");
      t.setAttribute("tabindex", on ? "0" : "-1");
    });
    var panels = widget.querySelectorAll(".ag-cli-install__panel");
  ...
  function onClick(event) {
    var tab = event.target.closest(".ag-cli-install__tab");
    if (!tab) return;
