...
$ uv run scripts/mcp_swap.py doctor --server agentgrep         # effective environment + footguns
$ uv run scripts/mcp_swap.py status --server agentgrep
$ uv run scripts/mcp_swap.py use-local --server agentgrep --env KEY=VALUE --dry-run
$ uv run scripts/mcp_swap.py use-local --server agentgrep --env KEY=VALUE
